...
export default function EmployeeAttendance({ employeeId }) {
  ...
  // ✅ Diferencia de horas si viene como "HH:mm:ss"
  const calculateHours = (checkIn, checkOut) => {
    ...
    if (diffMinutes < 0) return 'N/A'; // protección si hay error de datos
  ...
  const getStatusTextColor = (status) => {
    switch (status) {
      ...
      case 'LATE': return '#856404';
      case 'HALF_DAY': return '#0c5460';
      default: return '#383d41';
    }
  };

  if (loading) {
    return <div style={{ textAlign: 'center', padding: '50px' }}>Cargando asistencia...</div>;
  }
    ...
    <div>
      ...
      <div style={{ textAlign: 'center', marginBottom: '20px' }}>
        <button
          ...
          style={{ margin: '0 10px', padding: '10px 20px', background: '#28a745', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
        >
          Registrar Entrada
        </button>
        <button
          onClick={() => handleRegister("CHECK_OUT")}
          style={{ margin: '0 10px', padding: '10px 20px', background: '#dc3545', color: '#fff', border: 'none', borderRadius: '6px', cursor: 'pointer' }}
        >
          Registrar Salida
        </button>
      </div>
